...
def normalize_ini():
    ...
    with open(SETUP_INI, "r", encoding="utf-8") as f:
        lines = f.readlines()
    with open(SETUP_INI, "w", encoding="utf-8") as f:
        for line in lines:
...
def ensure_names():
    ...
    if os.path.exists(NAMES_INI):
        with open(NAMES_INI, "r", encoding="utf-8") as f:
            section = None
            for line in f:
                m = re.match(r"\s*\[(.+?)\]", line)
                if m:
    ...
    with open(NAMES_INI, "w", encoding="utf-8") as f:
